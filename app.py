...
# ==========================================
# 📱 2. الواجهة الرئيسية (تعديل العنوان الصارم على سطرين من أقصى اليمين لليسار)
# ==========================================
st.markdown(
    """
    <div style="width: 100%; text-align: right; direction: rtl; padding: 0px 5px; margin-top: -20px;">
        <span style="font-size: 32px; font-weight: 900; color: #00bfff; font-family: 'Courier New', monospace; letter-spacing: 1px; display: block;">ZEGAAR AMMAR</span>
        <span style="font-size: 32px; font-weight: 900; color: #00bfff; font-family: 'Courier New', monospace; letter-spacing: 1px; display: block; border-bottom: 2px solid rgba(0, 191, 255, 0.3); padding-bottom: 5px; margin-top: 2px;">GLASS MANAGER</span>
    </div>
...
selected_phone = st_searchbox(search_function=lambda q, **k: search_models_callback(q, unique_models), placeholder="🔍 ابحث عن هاتف أو اكتب اسماً جديداً...", key="phone_search_autocomplete_v9")
...
if st.session_state.custom_search_input:
    ...
    if size_grp:
        ...
        if final_size.strip() and final_panel.strip() and final_sensor.strip():
            if final_size in db_data and final_panel in db_data[final_size] and final_sensor in db_data[final_size][final_panel]:
                ...
                if matched_models_in_grp:
                    ...
                    with st.form("join_group_form_v9"):
        ...
        if final_size.strip() and final_panel.strip() and final_sensor.strip() and not has_matched_group:
            ...
            with st.form("create_new_group_form_v9"):
