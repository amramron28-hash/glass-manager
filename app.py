...
draw_neon_section(
    ...
    if final_size.strip():
        ...
        if final_panel == "➕ إضافة جديد":
            final_panel = st.text_input("اكتب شكل الشاشة")
    ...
    if final_size.strip() and final_panel.strip():
        ...
        if final_sensor == "➕ إضافة جديد":
            final_sensor = st.text_input("اكتب المستشعر")
    ...
    if final_size and final_panel and final_sensor:
        ...
        if models:
            st.success("🤝 توجد مجموعة مطابقة")
            st.write(models)
            if st.button("إضافة الهاتف للمجموعة"):
                add_model(final_size, final_panel, final_sensor, phone)
                st.session_state.show_success = "تمت الإضافة"
                st.rerun()
        else:
            st.warning("لا توجد مجموعة")
            if st.button("إنشاء مجموعة جديدة"):
                add_model(final_size, final_panel, final_sensor, phone)
