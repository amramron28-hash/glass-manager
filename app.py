...
selected_phone = st_searchbox(search_function=lambda q, **k: search_models_callback(q, unique_models), placeholder="🔍 ابحث عن هاتف أو اكتب اسماً جديداً...", key="phone_search_autocomplete_v7")
...
if not selected_phone:
    ...
    custom_typed = st.text_input(label="", placeholder="اكتب اسم الهاتف الجديد هنا...", key="manual_input_v7")
    if custom_typed.strip():
        st.session_state.custom_search_input = custom_typed.strip()

# ==========================================
# 🛑 تطبيق تتابع خطة المراحل 1، 2، 3 الصارم
# ==========================================
if st.session_state.custom_search_input:
    ...
    # 🟢 [المرحلة 1]: فحص وجود الاسم في السحابة
    size_grp, panel_grp, sensor_grp, real_name = find_model_coords(db_data, current_search)
    
    if size_grp:
        # وجد الهاتف ⬅️ يعرض النيون بالفارق الجديد (±0.03) وينتهي التتابع تماماً
        ...
        draw_neon_section("أكبر بقليل (فارق ±0.03)", compat_results["plus"], "#3498db", "➕", current_search)
        draw_neon_section("أصغر بقليل (فارق ±0.03)", compat_results["minus"], "#e67e22", "➖", current_search)
        draw_neon_section("مستشعر مختلف يحتاج حذر", compat_results["warn"], "#ef4444", "⚠️", current_search)
    
    else:
        # 🟡 [المرحلة 2]: لم يجد الاسم ⬅️ فتح نافذة خيارات النقر الحية للبحث عن عضوية ومطابقة
        ...
        # خيارات تظهر تضغط عليها (selectbox) مسحوبة حياً من مخزون السحابة مع حقل نصي للطوارئ
        sel_size = st.selectbox("📏 1. اختر مقاس الهاتف (Size):", [""] + live_sizes + ["➕ إضافة مقاس جديد يدوي"])
        
        # حقل طوارئ يفتح فقط إذا ضغطت على إضافة جديد
        final_size = st.text_input("✍️ اكتب المقاس الجديد يدويًا:") if sel_size == "➕ إضافة مقاس جديد يدوي" else sel_size

        final_panel = ""
        if final_size:
            sel_panel = st.selectbox("📺 2. اختر نوع الشاشة (Panel):", [""] + live_panels + ["➕ إضافة نوع شاشة جديد"])
            final_panel = st.text_input("✍️ اكتب نوع الشاشة الجديد:") if sel_panel == "➕ إضافة نوع شاشة جديد" else sel_panel

        final_sensor = ""
        if final_size and final_panel:
            sel_sensor = st.selectbox("👁️ 3. اختر مستشعر التقارب (Sensor):", [""] + live_sensors + ["➕ إضافة مستشعر جديد"])
            final_sensor = st.text_input("✍️ اكتب المستشعر الجديد:") if sel_sensor == "➕ إضافة مستشعر جديد" else sel_sensor

        # تشغيل فحص العضوية والمطابقة الفورية بعد اكتمال الضغط على الخيارات الثلاثة
        has_matched_group = False
        if final_size.strip() and final_panel.strip() and final_sensor.strip():
            
            # فحص هل التوليفة التي ضغطت عليها تملك مجموعة مسجلة مسبقاً؟
            if final_size in db_data and final_panel in db_data[final_size] and final_sensor in db_data[final_size][final_panel]:
                ...
                if matched_models_in_grp:
                    ...
                    st.success(f"🤝 وجد ما يطابقه! هذا الهاتف متوافق تماماً وينتمي لمجموعة الهواتف التالية: {matched_models_in_grp}")
                    ...
                    with st.form("join_group_form"):
                        ...
                        if st.form_submit_button("نعم، إدراج الهاتف كعضو في المجموعة 🔗"):
                            if add_model(final_size, final_panel, final_sensor, current_search):
                                st.session_state.show_success_toast = f"تم إدراج {current_search} كعضو بنجاح!"
                                st.session_state.custom_search_input = ""
                                st.rerun()

        # 🔴 [المرحلة 3]: نفذنا كل ما سبق ولم يظهر شيء (التوليفة جديدة بالكامل ولم تسجل مسبقاً)
        if final_size.strip() and final_panel.strip() and final_sensor.strip() and not has_matched_group:
            ...
            with st.form("create_new_group_form"):
                st.markdown(f"<b style='color:#ef4444;'>📂 إدراج الهاتف وتأسيس مجموعة جديدة بالكامل في السحابة:</b>", unsafe_allow_html=True)
                st.info(f"المواصفات المحجوزة للمجموعة الجديدة: المقاس ({final_size}) | الشاشة ({final_panel}) | المستشعر ({final_sensor})")
                if st.form_submit_button("تأكيد حجز وإنشاء المجموعة الجديدة 💾"):
                    if add_model(final_size, final_panel, final_sensor, current_search):
                        st.session_state.show_success_toast = f"تم تأسيس مجموعة فنية جديدة بنجاح وإدراج {current_search} كأول عضو فيها!"
                        st.session_state.custom_search_input = ""
                        st.rerun()

# 🛠️ اللوحة الجانبية (المراقب الصامت وعين الحراسة الحية)
with st.sidebar:
    st.markdown("<h2 style='text-align:right;color:#00bfff;'>🛠️ المراقب الصامت</h2>", unsafe_allow_html=True)
    st.markdown("---")
    with st.expander("⚙️ الإعدادات والمؤشرات الحية", expanded=True):
        ...
        if st.button("🧹 تشغيل الصيانة وتطهير السحابة"):
            ...
            if changes_made:
                ...
                st.toast("🎯 فحص المراقب الصامت: قاعدة البيانات السحابية نظيفة ومؤمنة تماماً.")
