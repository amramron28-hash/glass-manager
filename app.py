...
if phone and size_str and is_exact_match:
    ...
    st.markdown(
        ...
        📊 نتائج التوافق والمقاسات للهاتف: {real_name}
        </div>
        """,
        unsafe_allow_html=True
    )
...
# ============================================================
    ...
    st.warning(
        f"⚠️ الهاتف ({phone}) غير مسجل بالاسم الحرفي هذا. "
        "تم فتح الخطة 2 لإدخال مواصفاته يدوياً بالتتابع:"
    )
    ...
    with col_s:
        ...
        new_size = st.text_input(
            "📐 1. المقاس الرقمي للزبون (مثال: 6.67):",
            key="workflow_size"
        )
    ...
    if (
        new_size.strip()
        and str(new_panel).strip()
        and str(new_sensor).strip()
    ):
        ...
        if matched_list:
            ...
            st.markdown(
                f"🎯 الموديلات المتوافقة: {', '.join(matched_list)}"
            )
            ...
            st.error(
                "❌ خطة الطوارئ: لا توجد مجموعة تطابق هذه المواصفات"
            )
