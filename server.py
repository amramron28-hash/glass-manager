...
def server(input, output, session):
    ...
    def get_cached_stats_data():
        ...
        if now - _stats_time() < config.REFRESH_INTERVAL_SEC and _cached_stats() is not None:
        ...
        try:
            ...
            log.error(f"Stats Cache Error: {error}")
            return {}

    @reactive.calc
    def get_cached_status_data():
        now = __import__('time').time()
        if now - _status_time() < config.REFRESH_INTERVAL_SEC and _cached_status() is not None:
        ...
        try:
            ...
            log.error(f"Status Cache Error: {error}")
            return {}

    # ===== Watchers =====
    @reactive.effect
    def watcher_refresh():
        reactive.invalidate_later(config.REFRESH_INTERVAL_SEC)
        db_trigger()
        try:
            ...
            log.error(f"Watcher Refresh Error: {error}")

    @reactive.effect
    def watcher_status():
        reactive.invalidate_later(config.REFRESH_INTERVAL_SEC * 2)
        try:
            svs.execute_status_logic(get_cached_status_data)
        except Exception as error:
            log.error(f"Watcher Status Error: {error}")
    ...
    def handle_search():
        try:
            ...
            log.error(f"Search Handler Error: {error}")
    ...
    def run_plan(screen_size, panel_name, sensor_name, plan_type):
        try:
            ...
            log.error(f"Run Plan Error: {error}")
            plan_results.set(None)
    ...
    def trigger_save_model(action_name):
        try:
            ...
            if success:
                ...
                log.warning(f"Save failed for action: {action_name}")
        except Exception as error:
            log.error(f"Save Model Error: {error}")
    ...
    def results_workflow_view():
        ...
        try:
            ...
            log.error(f"Workflow Render Error: {error}")
            return ui.div(ui.HTML(f"⚠️ خطأ في عرض النتائج: {error}"))
