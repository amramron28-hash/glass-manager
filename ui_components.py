...
def draw_neon_section(title=None, models_list=None, color_hex="#00bfff", badge_icon="📱", plan_type="exact"):
    ...
    for model in models_list:
        cards.append(ui.HTML(f"""<div class="ammar-flat-card {card_class}" style="max-width:500px; margin-left:auto; margin-right:auto;"><div class="image-placeholder-box" style="border-color:{color_hex};">📱</div><div class="flat-phone-text">{escape(str(model))}</div></div>"""))
    return ui.div(*cards)
...
def draw_plan_3_modal(phone_name, size, panel, sensor):
    return ui.div(ui.div(ui.div(ui.h3("🚨 خطة الطوارئ 3: إنشاء مجموعة جديدة", style="color:#e67e22; text-align:center; margin-bottom:15px;"), ui.p(f"النظام لم يعثر على أي مجموعة مطابقة للمواصفات المدخلة للهاتف ({phone_name}).", style="text-align:center; color:#bbb; font-size:14px;"), ui.div(f"📐 المقاس المقترح: {size} | 📺 الشاشة: {panel} | 👁️ المستشعر: {sensor}", style="background:rgba(230,126,34,0.1); border:1px solid #e67e22; padding:10px; border-radius:8px; font-size:13px; text-align:center; margin-bottom:20px; color:#e67e22;"), ui.p("هل تريد تسجيل هذا الهاتف وتأسيس مرجع فني ومجموعة جديدة له في السحاب?", style="text-align:right; font-size:14px; margin-bottom:20px;"), ui.div(ui.input_action_button("p3_submit", "💾 نعم، أنشئ المجموعة واحفظ", class_="btn-neon", style="background:#e67e22; color:white; padding:12px; border:none; border-radius:8px; font-weight:bold; flex:2;"), ui.input_action_button("p3_cancel", "تراجع", class_="btn-neon", style="background:#7f8c8d; color:white; padding:12px; border:none; border-radius:8px; flex:1;"), style="display:flex; gap:10px;"), class_="glass-card", style="width:90%; max-width:450px; background:#161b22; border-color:#e67e22; border-width:2px; box-shadow: 0 0 25px rgba(230, 126, 34, 0.4);"), class_="custom-modal-backdrop"), id="plan_3_modal_container")
