...
def draw_neon_section(title, models, color, icon, cls): 
    # توزيع الألوان والنسب
    dot_colors = {"exact": "#2ecc71", "plus": "#3498db", "minus": "#e67e22", "warn": "#e74c3c"}
    percentages = {"exact": "0.00", "plus": "+0.03", "minus": "-0.03", "warn": "⚠️"}
    
    dot_color = dot_colors.get(cls, "#ffffff")
    percentage = percentages.get(cls, "")
    
    return ui.div(
        ui.div(
            ui.span("● ", style=f"color: {dot_color}; font-size: 14px;"),
            ui.strong(title),
            ui.span(f" ({percentage})", style="font-size: 11px; opacity: 0.7; margin-left: 5px;"),
...
# بقية الدوال (تم دمجها بنفس المنطق)
def draw_welcome_section(src="phone_image.webp"): return ui.div(ui.tags.img(src=src, class_="welcome-phone-image"), class_="glass-card-container")
def draw_technical_coords(size, panel, sensor, name): return ui.div(ui.h3(f"📱 {name}"), ui.p(f"المقاس: {size}"), ui.p(f"الشاشة: {panel}"), ui.p(f"المستشعر: {sensor}"), class_="glass-card-container")
def draw_plan_2_modal(p, pnl, sns): return ui.div(ui.h3("خطة 2"), ui.input_text("p2_size", "المقاس"), ui.input_selectize("p2_panel", "الشاشة", choices=pnl or []), ui.input_selectize("p2_sensor", "المستشعر", choices=sns or []), ui.input_action_button("exec_plan2", "بحث"), class_="glass-card-container")
def draw_plan_3_modal(p): return ui.div(ui.h3("🚨 خطة 3"), ui.input_action_button("btn_close_modal", "إغلاق"), class_="glass-card-container")
def draw_database_status(t): return ui.div(ui.div("إجمالي الهواتف", class_="metric-title"), ui.div(str(t), class_="metric-value"), class_="metric-box")
