...
def draw_technical_coords(size_grp, panel_grp, sensor_grp):
    """إظهار كرت الأبعاد الفنية التوافقية لهاتف الزبون في المنتصف"""
    st.markdown(f"""
...
def draw_neon_section(title, models_list, color_hex, badge_icon, current_search):
    """توليد نتائج التوافق بالبطاقات الملونة النيونية العريضة الأصلية كلياً"""
    if not models_list:
        return
            ...
            if comp_model.lower().strip() == current_search.lower().strip():
                st.markdown(f"""
                    <div style='background: linear-gradient(135deg, #0f172a, #1e293b); 
                                color: #00bfff; padding: 15px; border-radius: 8px; text-align: center; 
                ...
                st.markdown(f"""
                    <div style='background: linear-gradient(135deg, #1e293b, #0f172a); 
                                color: #e2e8f0; padding: 15px; border-radius: 8px; text-align: center; 
