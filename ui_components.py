...
def inject_pwa_and_styles():
    ...
    # إضافة رابط ملف CSS الخارجي + تنسيق الخلفية الديناميكي فقط
...
def _draw_plan_modal(title, title_color, phone_name, size_label, size_id, panel_label, panel_id, sensor_label, sensor_id, btn_label, btn_id, btn_color, panels, sensors):
    p_opts = {p:p for p in panels if p}
    s_opts = {s:s for s in sensors if s}
    return ui.div(ui.div(ui.div(
        ...
        ui.input_select(panel_id, panel_label, choices=p_opts),
        ui.input_select(sensor_id, sensor_label, choices=s_opts),
...
def draw_plan_2_modal(phone_name, panels, sensors):
    return _draw_plan_modal("📋 المواصفات الفنية لـ {phone_name}", "#3498db", phone_name, "📏 مقاس الشاشة:", "p2_size", "📺 نوع الشاشة:", "p2_panel", "👁️ نوع المستشعر:", "p2_sensor", "🔍 فحص المطابقة", "p2_search", "#2ecc71", panels, sensors)

def draw_plan_3_modal(phone_name, panels, sensors):
    return _draw_plan_modal("🔮 الخطة البديلة المتقدمة لـ {phone_name}", "#e67e22", phone_name, "📏 المقاس المقترح:", "p3_size", "📺 تخصيص نوع الشاشة:", "p3_panel", "👁️ تخصيص المستشعر:", "p3_sensor", "⚡ تشغيل البحث الذكي", "p3_search", "#e67e22", panels, sensors)
