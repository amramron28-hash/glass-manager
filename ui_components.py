...
html, body, .container-fluid {{
    background-color:#0a0e17 !important; background-image: linear-gradient(rgba(10,14,23,.20), rgba(10,14,23,.20)), url('data:image/webp;base64,{_bg_cache}');
    background-size:92% auto !important; background-position:center center !important; background-repeat:no-repeat !important; background-attachment:fixed !important;
    color:white !important; direction:rtl !important;
}}
...
.search-box {{ position:relative; width:90%; max-width:500px; margin:35px auto; }}
input[type="text"], input[type="number"], select {{ width:100%; background:#111827 !important; color:white !important; border:1px solid #00bfff !important; border-radius:12px !important; padding:12px !important; direction:rtl; }}
...
.ammar-flat-card {{ display:flex; justify-content:space-between; align-items:center; padding:14px; margin-bottom:12px; border-radius:12px; width:100%; box-sizing:border-box; }}
.flat-exact {{ background: linear-gradient(135deg,#0d1f13,#07120b); border: 2px solid #2ecc71; box-shadow: 0 0 10px rgba(46,204,113,0.2); }}
.flat-plus {{ background: linear-gradient(135deg,#0b1a33,#060e1c); border: 2px solid #3498db; box-shadow: 0 0 10px rgba(52,152,219,0.2); }}
.flat-minus {{ background: linear-gradient(135deg,#2b1807,#140b03); border: 2px solid #e67e22; box-shadow: 0 0 10px rgba(230,126,34,0.2); }}
...
.flat-phone-text {{ color:white; font-size:20px; font-weight:800; text-align: right; }}
.image-placeholder-box {{ width:50px; height:50px; border-radius:8px; border: 1px dashed #00bfff; display:flex; align-items:center; justify-content:center; }}
...
<h3 style="color:#00bfff; text-align:center; margin-bottom:15px;">📱 {escape(str(model_name))}</h3>
...
def draw_neon_section(title=None, models_list=None, color_hex="#00bfff", badge_icon="📱", plan_type="exact"):
    if models_list is None and isinstance(title, list): models_list = title; title = "الهواتف المتوافقة"
...
<div class="ammar-flat-card {card_class}" style="max-width:500px; margin-left:auto; margin-right:auto;">
    <div class="image-placeholder-box" style="border-color:{color_hex};">📱</div>
    <div class="flat-phone-text">{escape(str(model))}</div>
</div>
"""))
    return ui.div(*cards)
...
def draw_plan_2_modal(phone_name, existing_panels, existing_sensors):
    ...
    return ui.div(ui.div(ui.div(
                ...
                ui.div(ui.input_select("p2_panel", "📺 اختر شكل ونوع الشاشة:", choices=panel_options), ui.input_action_button("btn_add_panel", "➕", class_="btn-neon", style="padding:10px; margin-top:24px; background:#3498db; border:none; color:white; border-radius:8px;"), style="display:flex; gap:5px; width:100%; margin-bottom:15px; align-items: center;"),
                ui.div(ui.input_select("p2_sensor", "👁️ اختر نوع مستشعر التقارب الصارم:", choices=sensor_options), ui.input_action_button("btn_add_sensor", "➕", class_="btn-neon", style="padding:10px; margin-top:24px; background:#3498db; border:none; color:white; border-radius:8px;"), style="display:flex; gap:5px; width:100%; margin-bottom:20px; align-items: center;"),
                ui.div(ui.input_action_button("p2_search", "🔍 فحص وتطابق المجموعات", class_="btn-neon", style="background:#2ecc71; color:white; padding:12px; border:none; border-radius:8px; font-weight:bold; flex:2;"), ui.input_action_button("p2_cancel", "إلغلاق", class_="btn-neon", style="background:#e74c3c; color:white; padding:12px; border:none; border-radius:8px; flex:1;"), style="display:flex; gap:10px; justify-content:space-between;"),
            ...
            ), class_="custom-modal-backdrop"), id="plan_2_modal_container")
...
def draw_plan_3_modal(phone_name, size, panel, sensor):
    return ui.div(ui.div(ui.div(
                ...
                ui.div(ui.input_action_button("p3_submit", "💾 نعم، أنشئ المجموعة واحفظ", class_="btn-neon", style="background:#e67e22; color:white; padding:12px; border:none; border-radius:8px; font-weight:bold; flex:2;"), ui.input_action_button("p3_cancel", "تراجع", class_="btn-neon", style="background:#7f8c8d; color:white; padding:12px; border:none; border-radius:8px; flex:1;"), style="display:flex; gap:10px;"),
            ...
            ), class_="custom-modal-backdrop"), id="plan_3_modal_container")

def draw_control_panel(total_models=0, empty_groups_count=0):
    return ui.div(ui.h3("🛠️ لوحة التحكم", style="color:#00bfff;text-align:center;"), ui.div(f"📱 الهواتف المسجلة: {total_models}", class_="glass-card"), ui.div(f"🧹 مجموعات للمراجعة: {empty_groups_count}", class_="glass-card"))
