...
def draw_plan_2_modal(phone_name, existing_panels, existing_sensors):
    ...
    return ui.div(ui.div(ui.div(ui.h3(f"📋 المواصفات الفنية لـ {phone_name}", style="color:#3498db; text-align:center;"), ui.input_numeric("p2_size", "📏 مقاس الشاشة:", value=None, step=0.01), ui.input_select("p2_panel", "📺 نوع الشاشة:", choices=panel_options), ui.input_select("p2_sensor", "👁️ نوع المستشعر:", choices=sensor_options), ui.input_action_button("p2_search", "🔍 فحص المطابقة", class_="btn-neon", style="width:100%; background:#2ecc71; color:white; padding:12px; border-radius:8px; border:none;"), class_="glass-card", style="width:90%; max-width:500px; background:rgba(22,27,34,.98);"), class_="custom-modal-backdrop"))
