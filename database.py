...
def load_db():
    try:
        # جلب البيانات مباشرة من السحابة بنظام الاستعلام الآمن
        res = supabase.table("phones").select("*").execute()
        rows = res.data or []
        
        # ⚡ إذا كانت السحابة فارغة (أو بها فقط هاتف الاختبار يدوياً)، يقوم الكود بضخ الـ 176 هاتفاً كاملاً بضربة واحدة في ثانية
        if len(rows) <= 1:
            try:
                bulk_records = []
                ...
                if bulk_records:
                    ...
                    # إعادة الاستعلام النهائي الحصري للامتلاء السحابي الكامل
                    res = supabase.table("phones").select("*").execute()
                    rows = res.data or []
            except Exception:
                pass
