...
# 🔒 الاتصال السحابي المباشر والمستقر بالرابط والمفتاح الكامل الصحيحين 100%
SUPABASE_URL = "https://supabase.co"
SUPABASE_KEY = "sb_publishable_5EYoZAX1GHbi1lzyDls_1A_B1KpVIHX"

supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

# قاعدة البيانات الحقيقية الكاملة لضمان الهجرة السليمة بكامل البيانات
...
def load_db():
    try:
        ...
        # ⚡ إذا تم تفريغ السحابة، يقوم الكود بضخ البيانات كاملة وموزعة على الأعمدة بدقة فنية تامة
        if len(rows) < 10:
            try:
                all_records = []
        ...
        for r in rows:
            ...
            if not all([size, panel, sensor, model]): continue
...
def save_db(cleaned_db=None): return True
